...
def check_format_and_calculate_performance(file_path):
    # 加载Excel文件
    ...
    for row in range(start_row, sheet.max_row + 1):
        if sheet.cell(row=row, column=1).value == "创新能力":
            start_personal = row
        if sheet.cell(row=row, column=1).value == "工作计划":
            end_personal = row
            break
    ...
    personal_score = sum(personal_scores) / 2.0
    ...
    return Member(name, final_time_score, final_quality_score, task_score, personal_score, final_score, overtime_penalty)

class Member:
    def __init__(self, name, final_time_score, final_quality_score, task_score, personal_score, final_score, overtime_penalty):
        self.name = name
    ...
    def show(self):
        output = (f"{self.name}： \t总完成时间分：{self.final_time_score:.2f}，总完成质量分：{self.final_quality_score:.2f}，"
                  ...
                  f"最终绩效分：{self.final_score:.2f}，勤勉度扣分：{self.overtime_penalty}")
        print(output)
...
# 示例用法
directory = '/home/zhenghaoxu@geometricalpal.com/PycharmProjects/pythonProject/data/2025Q4-manager'  # 替换为你的文件夹路径
xlsx_files = find_xlsx_files(directory)
member_list = []
for file_path in xlsx_files:
    member = check_format_and_calculate_performance(file_path)
